...
def parse_args():
    ...
    ap.add_argument("--questions", type=str,
                    default="/home/user/SSD_adapters/kangaroo_data/question.jsonl",
                    help="MT-Bench style JSONL with 'turns' field.")
    ap.add_argument("--output", type=str, default="/tmp/adapter_only_out.jsonl",
...
def main():
    ...
    for i, q in enumerate(questions):
        if hasattr(model, "reset_status"):
            model.reset_status()
        messages = build_chat_messages(q["turns"], args.system_prompt)
        ...
        if ar_record is not None:
            record["ar_baseline"] = ar_record
            record["speedup_decode_tps"] = (
        ...
        print(f"  Q: {q['turns'][0][:120]}...")
        print(f"  A (adapter-only, {n_new} tok @ {decode_tps:.1f} tok/s): {reply_text[:200]}...")
        if ar_record is not None:
            print(f"  A (AR full,     {ar_record['tokens']} tok @ {ar_record['tps']:.1f} tok/s): {ar_record['text'][:200]}...")
            print(f"  Speedup adapter-only vs AR: {record['speedup_decode_tps']:.2f}x")
